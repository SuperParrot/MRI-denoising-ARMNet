...
class Interface():
    ...
    def train(self, epoch=1, learning_rate=1e-5, batch_size=1, save_freq=1, shuffle=True, noise_level=None, train_mask=False):
        ...
        for i in range(epoch):
            ...
            for batch_idx, (input, label, mask) in enumerate(train_loader):
                loss_rec, loss_dice = self.train_batch(input, label, mask, train_mask)
                ...
                if (last_time > 0):
                    ...
                    if ((batch_idx % 100 == 0 and batch_idx > 0) or batch_idx >= batchNum - 1):
                        print("epoch:%d/%d batch:%d/%d\tloss:%.4lf %.4lf ETA:%02dH:%02dM:%02dS" % (
                        i + 1, epoch, batch_idx + 1, batchNum, loss_rec, loss_dice, h, m, s))
                else:
                    print("epoch:%d/%d batch:%d/%d\tloss:%.4lf %.4lf ETA:??H:??M:??S" % (
                    i + 1, epoch, batch_idx + 1, batchNum, loss_rec, loss_dice))
    ...
    def train_batch(self, input, label, mask, train_mask):
        if (torch.cuda.is_available()):
            input = input.to(self.device)
            label = label.to(self.device)
            mask = mask.to(self.device)

        pred, mask_pred = self.net(input, train_mask)
        ...
        for batch_idx, (input, label, mask) in enumerate(eval_loader):
            ...
            if (assigned_modelNames != None):
                ...
                for model_name in assigned_modelNames:
                    ...
                    with torch.no_grad():
                        pred, pred_mask = self.net(input, False)
            ...
            '''
            plt.imshow(pred[0,0].cpu(), cmap='gray', vmin=0, vmax=1)
            plt.show()

            plt.imshow(label[0, 0].cpu(), cmap='gray', vmin=0, vmax=1)
            plt.show()
            exit(0)
            '''
    ...
    def predict(self, file_name, assigned_modelNames=None, noise_level=None):
        ...
        test_loader = DataLoader(dataset=test_data, batch_size=8, shuffle=False, num_workers=2, pin_memory=False)
