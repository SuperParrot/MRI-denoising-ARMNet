...
class MultiDila_Block(nn.Module):
    def __init__(self, params):
        ...
        self.ln_nl=nn.Sequential(
            ...
            nn.LayerNorm([H, W], elementwise_affine=False),
            nn.GELU(),
        )
...
class MultiDilaResnet(nn.Module):
    def __init__(self, C_in, C_out, num_layers, height, width, features=64):
        super(MultiDilaResnet, self).__init__()

        self.backbone=nn.Sequential(
            nn.Conv2d(in_channels=C_in, out_channels=features, kernel_size=1, stride=1, padding=0, bias=False),
            nn.GELU(),
        )

        self.resnet=ResNet(num_layers, MultiDila_Block, features, height, width)
